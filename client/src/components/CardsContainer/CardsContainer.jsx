...
export default function CardsContainer () {
    ...
    //Mandar al inicio cuando se busca.
    useEffect(()=>{
        setCurrentPage(1)
    }, [countriesSorted]);


    //Creamos un array con el numero de pages y asignanmos su valor en base al index
    ...
    return(
        <div>
            ...
            <input className={style.nextPrevButtons} type="button" value="Prev" name="Prev" onClick={prevHandler} />

             {pageNumbers.map(page => (
                <button className={(page === currentPage)? style.currentButton:style.numberButtons} key={page} onClick={()=> setCurrentPage(page)} >{page}</button>
             ))}

            <input className={style.nextPrevButtons} type="button" value="Next" name="Next" onClick={nextHandler} />
